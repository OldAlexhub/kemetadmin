...
// import AssignedSightSeeings from "../components/AssignedSightSeeings";
import Logo from "../images/logo.png"; // Ensure path is correct
import AssignedReturns from "../components/AssignedReturns";
import AcceptedSight from "../components/AcceptedSight";
...
const AssignedTrips = () => {
  ...
  useEffect(() => {
    const fetchTrips = async () => {
      try {
        ...
        const response = await axios.get(process.env.REACT_APP_ASSIGNED_TRIPS, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        setAssigns(response.data.assigned);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const handleUnassign = async (assign) => {
    try {
      ...
      const url = `${process.env.REACT_APP_UNASSIGN_TRIPS}/${assign._id}`;
  ...
  return (
    <div>
          ...
          <tbody>
            {assigns.map((assign) => (
              <tr key={assign._id}>
                ...
                <td>{new Date(assign.pickupDate).toLocaleDateString()}</td>
                <td>{assign.pickupTime}</td>
      ...
      <hr />
      <AcceptedSight />
      <div className="table-responsive">{/* <AssignedSightSeeings /> */}</div>
    </div>
  );
};
